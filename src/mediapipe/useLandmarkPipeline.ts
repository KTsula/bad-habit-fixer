...
const POLL_INTERVAL_MS = 1000 / 20; // setInterval cadence (~20 FPS target)
  ...
  useEffect(() => {
    ...
    createLandmarkers()
      .then((lm) => {
        ...
        setStatus('running');
        intervalId = setInterval(processFrame, POLL_INTERVAL_MS);
      })
      .catch((err: unknown) => {
        if (cancelled) return;
    ...
    function processFrame(): void {
      ...
      if (elapsed < MIN_FRAME_INTERVAL_MS) return;
